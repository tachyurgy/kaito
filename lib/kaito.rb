...
module Kaito
  class << self
    ...
    # @param tokenizer [Symbol] tokenizer to use (:gpt35_turbo, :gpt4, :claude, :character)
    ...
    def split(text, strategy: :semantic, max_tokens: 512, overlap_tokens: 0, tokenizer: :gpt4, **options)
      splitter = create_splitter(strategy, max_tokens: max_tokens, overlap_tokens: overlap_tokens, tokenizer: tokenizer, **options)
      splitter.split(text)
    end
    ...
    #     puts chunk.text
    #   end
    def stream_file(file_path, strategy: :semantic, max_tokens: 512, overlap_tokens: 0, tokenizer: :gpt4, **options, &block)
      splitter = create_splitter(strategy, max_tokens: max_tokens, overlap_tokens: overlap_tokens, tokenizer: tokenizer, **options)
    ...
    def create_tokenizer(tokenizer_name)
      ...
      when :character
        ...
        raise ArgumentError, "Unknown tokenizer: #{tokenizer_name}"
      end
    end
  end
